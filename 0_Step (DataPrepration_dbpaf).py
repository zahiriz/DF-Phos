...
class DataPrepration():
    ...
    def ReadProteinsSequence_DBPAF(self,filename):
        df = pd.read_excel(filename, sheet_name='Sheet1')
        data = df.to_numpy()
        for i,item in enumerate(data):
            if (not(item[1] in self.proteinlist.keys())):
                protein = Proteins()
                protein.acc = item[1]
                protein.sequence =item[4]
                protein.position.append(item[2])
                protein.phostype.append(item[3])
                protein.species =item[5]
                self.proteinlist[protein.acc] =protein
            else:
                self.proteinlist[item[1]].position.append(item[2])
                self.proteinlist[item[1]].phostype.append(item[3])
        return self.proteinlist
    def SlideStringToSequence(self,windowsize):
        filePho = open("PreparedData/dbpaf/dbpaf_Phos_windows_{}.fasta".format(windowsize), "w")
        fileNonPho = open("PreparedData/dbpaf/dbpaf_NonPhos_windows_{}.fasta".format(windowsize), "w")
...
elm = prepare.ReadProteinsSequence_DBPAF("DataSet/dbpaf/Dataset.xlsx")
prepare.FrequencyOfPhospho()
prepare.SlideStringToSequence(33)
